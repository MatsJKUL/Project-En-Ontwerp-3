...
hit_rect.topleft = (10, 10)
double_rect.topleft = (150,10)
stand_rect.topleft = (60,10)
again_rect.center = (WIDTH // 2 - 150, HEIGHT // 2)
stop_rect.center = (WIDTH // 2 + 50, HEIGHT // 2)
...
while running:
    ...
    for number in range(len(players)):
        screen.blit(card_images[players[number].status()[0][0]], ((number+1) * WIDTH//(len(players)+1) -60, 500))
        pygame.display.update()
        clock.tick(30)
    ...
    clock.tick(30)
    for number in range(len(players)):
        screen.blit(card_images[players[number].status()[0][1]], ((number+1) * WIDTH//(len(players)+1) + (len(players[number].status()[0])-1)*30 -60, 500))
        pygame.display.update()
        clock.tick(30)
        pygame.time.delay(500)
    dealer.get_card(random_card_choice(cards, deleted_cards))
    ...
    for number in range(len(players)):
        ...
        move = None
        pygame.draw.rect(screen, (50, 50, 50), hit_rect)
        screen.blit(button_hit, hit_rect.topleft)
        pygame.draw.rect(screen, (50, 50, 50), double_rect)
        screen.blit(button_double, double_rect.topleft)
        pygame.draw.rect(screen, (50, 50, 50), stand_rect)
    ...
    while dealer_takes_card:
        ...
        if dealer_score > 21:
            ...
            dealer.get_card(random_card_choice(cards, deleted_cards))
            screen.blit(card_images[dealer.status()[0][-1]], (550 + (len(dealer.status()[0]) - 1) * 30, 30))
            pygame.display.update()
            clock.tick(30)
    # Score players
    ...
    screen.blit(button_again, again_rect.center)
    screen.blit(button_stop, stop_rect.center)
    pygame.display.update()
    clock.tick(30)
    while play_again:
        for event in pygame.event.get():
